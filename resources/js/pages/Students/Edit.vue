...
defineOptions({
    layout: {
        breadcrumbs: [
            ...
            {
                title: 'Edit',
                href: students.edit({ student: props.student.id }),
            },
        ],
    },
});
...
const submit = () => {
    form.put(students.update({ student: props.student.id }).url);
};
</script>
        ...
        <div class="rounded-md border bg-white dark:bg-zinc-950 p-6">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex items-center gap-4">
                    <Button type="submit" :disabled="form.processing">Update Student</Button>
                </div>
            </form>
        </div>
    </div>
